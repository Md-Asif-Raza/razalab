...
const CLIENTS = [
  { 
    id: 1, 
    name: 'Dank Drops', 
    category: 'Viral Distribution', 
    result: '+114%', 
    price: '$4,500',
    description: 'We deployed a network of 50 viral clippers to distribute Dank Drops product launches. Resulted in 12M+ organic views in 30 days.',
    graphData: [20, 35, 25, 45, 60, 85, 114],
  ...
  { 
    ...
    result: '+88%', 
    price: '$6,200',
    description: 'Scaled Raza AI awareness through strategic short-form educational content. Generated over 8.5M views and 40k signups.',
    graphData: [10, 15, 30, 40, 55, 70, 88],
  ...
  { 
    ...
    result: '+210%', 
    price: '$3,800',
    description: 'Automated UGC distribution for Nectar e-commerce. Achieved 210% increase in social traffic and significant ROAS boost.',
    graphData: [15, 45, 80, 120, 160, 190, 210],
  ...
  { 
    ...
    result: '+156%', 
    price: '$5,500',
    description: 'Strategic artist positioning through trend-aligned clips. Scaled Glitch from 10k to 250k followers in 3 months.',
    graphData: [5, 25, 50, 75, 100, 130, 156],
    img: 'https://images.unsplash.com/photo-1633167606207-38433714614f?w=800&q=80' 
  },
  { 
    id: 5, 
    name: 'Aether', 
    ...
    result: '+94%', 
    price: '$7,000',
    description: 'Applied the distribution engine to Aether SaaS features. Generated consistent monthly organic reach of 5M+ views.',
    graphData: [30, 40, 35, 55, 65, 80, 94],
    img: 'https://images.unsplash.com/photo-1620121692029-d088224efc74?w=800&q=80' 
  },
];
...
function ClientCard({ client, onClick }: { client: typeof CLIENTS[0]; onClick: () => void }) {
  return (
    <motion.div 
      className="client-crystal-card" 
      style={{ minWidth: '380px', height: '480px', cursor: 'pointer' }} 
      onClick={onClick}
      whileHover={{ y: -10 }}
    >
      <div className="crystal-media-wrapper" style={{ height: '60%' }}>
        <img src={client.img} alt={client.name} className="crystal-img" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
...
export default function SectionCampaigns() {
  ...
  const marqueeItems = useMemo(() => [...CLIENTS, ...CLIENTS, ...CLIENTS], []);
        ...
        {selectedClient && (
          <motion.div 
            ...
            style={{ 
              ...
              backdropFilter: 'blur(32px)', zIndex: 10000, 
          ...
          >
            <motion.div 
              ...
              style={{ 
                maxWidth: '1100px', width: '100%', background: '#050304', 
                borderRadius: '40px', border: '1px solid rgba(255,255,255,0.08)', 
                overflow: 'hidden', display: 'grid', gridTemplateColumns: '1.2fr 1fr',
                boxShadow: '0 50px 150px rgba(0,0,0,1), 0 0 50px rgba(0,230,118,0.05)'
              }}
              onClick={(e) => e.stopPropagation()}
            >
              <div style={{ height: '100%', minHeight: '600px', position: 'relative' }}>
                <img src={selectedClient.img} alt={selectedClient.name} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                <div style={{ position: 'absolute', bottom: 40, left: 40, right: 40, background: 'rgba(5,3,4,0.6)', padding: '24px', borderRadius: '24px', backdropFilter: 'blur(12px)', border: '1px solid rgba(255,255,255,0.1)' }}>
                   ...
                   <AnalyticsGraph data={selectedClient.graphData} color="var(--aura-neon)" height={120} />
                </div>
              </div>
              <div style={{ padding: '60px', display: 'flex', flexDirection: 'column' }}>
                <div style={{ color: 'var(--aura-neon)', fontSize: '0.8rem', fontWeight: 800, letterSpacing: '4px', textTransform: 'uppercase', marginBottom: '16px' }}>
                  {selectedClient.category}
                </div>
                <h2 style={{ fontSize: '3.5rem', fontWeight: 800, color: '#fff', marginBottom: '24px', textAlign: 'left', letterSpacing: '-2px' }}>
                  {selectedClient.name}
                </h2>
                <p style={{ fontSize: '1.15rem', opacity: 0.6, lineHeight: '1.7', marginBottom: '48px' }}>
